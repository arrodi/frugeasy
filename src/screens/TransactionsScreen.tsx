...
function BudgetSwipeRow({ budget, currency, darkMode, onSaveBudget, onDeleteBudget }: BudgetSwipeRowProps) {
  ...
  const snapTo = (x: number) => Animated.spring(translateX, { toValue: x, useNativeDriver: true, bounciness: 0 }).start();

  const panResponder = useMemo(
    () => PanResponder.create({
      onMoveShouldSetPanResponder: (_, g) => Math.abs(g.dx) > 12 && Math.abs(g.dx) > Math.abs(g.dy),
      onPanResponderMove: (_, g) => translateX.setValue(Math.max(-92, Math.min(92, g.dx))),
      onPanResponderRelease: (_, g) => {
        if (g.dx > 36) snapTo(84);
        else if (g.dx < -36) snapTo(-84);
        else snapTo(0);
      },
  ...
  return (
    <View style={styles.swipeRowWrap}>
      <View style={styles.swipeActionsBg} pointerEvents="box-none">
        <Pressable style={[styles.swipeActionLeft, darkMode && styles.listRowDark]} onPress={() => setEditing(true)}>
          <Text style={styles.actionBtnText}>Edit</Text>
        </Pressable>
        <Pressable
          style={[styles.swipeActionRight, darkMode && styles.listRowDark]}
          onPress={async () => {
            if (!editing) {
              setEditing(true);
              return;
      ...
      <Animated.View style={[styles.listRow, darkMode && styles.listRowDark, { transform: [{ translateX }] }]} {...panResponder.panHandlers}>
        <View style={styles.topRow}>
          ...
          {!editing ? (
            ...
            <TextInput
              ...
              style={[styles.smallInput, darkMode && styles.inputDark, styles.budgetInlineInput]}
        ...
        <View style={styles.inlineRow}>
          <Pressable style={styles.deleteBtn} onPress={() => onDeleteBudget(budget.id)}><Text style={styles.deleteBtnText}>Delete</Text></Pressable>
        </View>
      </Animated.View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  budgetInlineInput: { minWidth: 120, textAlign: 'right' },
  ...
  swipeRowWrap: { position: 'relative', marginBottom: 8 },
  swipeActionsBg: { position: 'absolute', inset: 0, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  swipeActionLeft: { marginLeft: 8, backgroundColor: '#e6f8ec', borderWidth: 1, borderColor: '#a9e6b7', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8 },
  swipeActionRight: { marginRight: 8, backgroundColor: '#e6f8ec', borderWidth: 1, borderColor: '#a9e6b7', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8 },
